...
for k=2:1:len
    ...
    if (k>2999) && (k<4000)
        w=[0; 0.3*sin(2*pi*((k/100)-40))*exp(-((k/100)-40)/10); 0]; % vehicles will collide when disturbance amplitude>0.3
        %w=[0; 0.5 ; 0];
        x0(:,k)=x0(:,k-1)+(1/6)*(k1+2*k2+2*k3+k4)-w;
    else
...
hold on
grid on
title({"Vehicles' Displacements over Time";"with Noise Disturbance"})
...
hold on
grid on
title({"Vehicles' Velocities over Time";"with Noise Disturbance"})
xlabel('Time (second)','Fontname', 'Times New Roman','fontsize',10)
...
title({"Vehicles' Accelerations over Time";"with Noise Disturbance"})
xlabel('Time (second)','Fontname', 'Times New Roman','fontsize',10)
...
title({"Vehicles' Locations over Time";"with Noise Disturbance"})
xlabel('Time (second)','Fontname', 'Times New Roman','fontsize',10)
...
title({"Vehicles' Spacing over Time";"with Noise Disturbance"})
xlabel('Time (second)','Fontname', 'Times New Roman','fontsize',10)
...
hold on
grid on
title({"Vehicles' Errors over Time";"with Noise Disturbance"})
axis tight
