...
% Including Heterogeneous Communication Time Delay
...
%% Constants
alpha=5; % acceleration constant
beta=4; % velocity constant
gamma=2; % displacement constant
...
for k=2:1:len
    ...
    %%%%%%%%%%%%%%%%%%%%%
        ...
        u1(:,k)=-((alpha*(a1(:,k)-a0(:,k-d1)) + beta*(v1(:,k)-v0(:,k-d1)) + gamma*(x1(:,k)-x0(:,k-d1)-r1))+(alpha*(a1(:,k)-a0(:,k-d1)) + beta*(v1(:,k)-v0(:,k-d1)) + gamma*(x1(:,k)-x0(:,k-d1)-r1)));
        u2(:,k)=-(alpha*(a2(:,k)-a1(:,k-d2)) + beta*(v2(:,k)-v1(:,k-d2)) + gamma*(x2(:,k)-x1(:,k-d2)-r2));
        u3(:,k)=-(alpha*(a3(:,k)-a2(:,k-d3)) + beta*(v3(:,k)-v2(:,k-d3)) + gamma*(x3(:,k)-x2(:,k-d3)-r3));
        u4(:,k)=-(alpha*(a4(:,k)-a3(:,k-d4)) + beta*(v4(:,k)-v3(:,k-d4)) + gamma*(x4(:,k)-x3(:,k-d4)-r4));
        u5(:,k)=-(alpha*(a5(:,k)-a4(:,k-d5)) + beta*(v5(:,k)-v4(:,k-d5)) + gamma*(x5(:,k)-x4(:,k-d5)-r5));
        u6(:,k)=-(alpha*(a6(:,k)-a5(:,k-d6)) + beta*(v6(:,k)-v5(:,k-d6)) + gamma*(x6(:,k)-x5(:,k-d6)-r6));
...
ylabel('The distance of the vehicles','Fontname', 'Times New Roman','fontsize',10)
